**OVERVIEW:**

 A simple python parser that pulls the timestamp, host, pid, user, ip, and port from the provided sample-auth.log and save the output in a csv file and print on screen.

**Dependencies:**

- Ubuntu 26.04 LTS
- Python 3
- sample-auth.log

**Installing:**

1. Download dependencies, logparser.py, and sample-auth.log. 
2. Place logparser.py and sample-auth.log in the same folder
3. Open up logparser.py with an text editor and change LOG_FILE and OUTPUT_CVS file path. 
          - LOG_FILE should be file pathed to where you put the sample-auth.log.
          - OUTPUT_CVS should be where you want the CVS out to show up as.
4. Run logparser.py
